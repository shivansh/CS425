...
void handle_client_connection(uint16_t sockfd) {
    ...
    while (1) {
        ...
        if (!safe_read(sockfd, buffer)) break;
        ...
        while (line = read_line(user_fp)) {
            if (strstr(line, username)) {
                ...
                if (strstr(line, password)) {
                    snprintf(buffer, strlen(username) + 7, "Hello %s",
                             username);
                    send(sockfd, buffer, BUFSIZE, 0);

                    /* Client will now send the filename. */
                    bzero(buffer, strlen(username) + 7);
                    if (!(bytes_read = safe_read(sockfd, buffer))) break;
...
int main(int argc, char **argv) {
    ...
    if (listen(sockfd, MAX_CONN) == 0) printf("Listening on port %d\n", port);
